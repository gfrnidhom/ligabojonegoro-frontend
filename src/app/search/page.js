  ...
  return (
    <div className="page-container animate-fade-in" style={{ maxWidth: 800, margin: '24px auto', padding: '0 16px 80px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        ...
        <span style={{ fontSize: 18, fontWeight: 800, color: '#0f172a' }}>Hasil Pencarian</span>
      </div>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <input 
            ...
            style={{
              ...
              padding: '0 24px 0 54px', color: '#0f172a', fontSize: 15, fontWeight: 600, outline: 'none',
      ...
      {loading ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '60px 0' }}>
          ...
          <span style={{ marginTop: 16, color: '#64748b', fontSize: 14, fontWeight: 500 }}>Mencari "{query}"...</span>
        </div>
      ) : !query ? (
        <div style={{ textAlign: 'center', padding: '60px 24px', color: '#64748b' }}>
          <Search size={48} style={{ margin: '0 auto 16px', opacity: 0.15, color: '#0f172a' }} />
          <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', marginBottom: 8 }}>Mulai Pencarian</h2>
          <p style={{ fontSize: 14 }}>Ketikkan kata kunci untuk mencari turnamen, tim, atau pemain.</p>
        </div>
      ) : totalResults === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px 24px', color: '#64748b' }}>
          <Search size={48} style={{ margin: '0 auto 16px', opacity: 0.15, color: '#0f172a' }} />
          <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', marginBottom: 8 }}>Tidak Ada Hasil</h2>
                ...
                {results.tournaments.map(t => (
                  <Link href={`/tournaments/${t.uuid || t.id}`} key={t.id} style={{ textDecoration: 'none' }}>
                    ...
                    onMouseLeave={e => {
                      ...
                      <div>
                        <h4 style={{ fontSize: 14, fontWeight: 700, color: '#0f172a', marginBottom: 4 }}>{t.name}</h4>
                        <span style={{ fontSize: 12, color: '#64748b' }}>{t.location || 'Bojonegoro'}</span>
                      </div>
                    </div>
                  </Link>
                ))}
                ...
                {results.teams.map(t => (
                  <Link href={`/teams/${t.uuid || t.id}`} key={t.id} style={{ textDecoration: 'none' }}>
                    ...
                    onMouseLeave={e => {
                      ...
                      <div>
                        <h4 style={{ fontSize: 14, fontWeight: 700, color: '#0f172a', marginBottom: 4 }}>{t.name}</h4>
                        <span style={{ fontSize: 12, color: '#64748b' }}>{t.sport?.name || 'Cabang Umum'}</span>
                      </div>
                    </div>
                  </Link>
                ))}
                  ...
                  <Link href={`/players/${p.uuid || p.id}`} key={p.id} style={{ textDecoration: 'none' }}>
                    ...
                    onMouseLeave={e => {
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <h4 style={{ fontSize: 14, fontWeight: 700, color: '#0f172a', marginBottom: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.name}</h4>
                        <span style={{ fontSize: 12, color: '#64748b', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.team?.name || '-'}</span>
                      </div>
                      {p.position && (
                         <div style={{ padding: '4px 8px', borderRadius: 8, background: 'rgba(59,130,246,0.08)', color: '#3b82f6', fontSize: 10, fontWeight: 800 }}>
                           {p.position}
                         </div>
                      )}
                    </div>
  ...
  return (
    <Suspense fallback={
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '100px 0' }}>
        <div className="loader"></div>
        <span style={{ marginTop: 16, color: '#64748b', fontSize: 14, fontWeight: 500 }}>Memuat pencarian...</span>
      </div>
    }>
      <SearchResults />
    </Suspense>
